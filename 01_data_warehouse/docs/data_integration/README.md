...
The published [data integration model](data_integration_model.webp) documents the source relationships and derived keys prepared in Silver. It is an integration-discovery artifact, not a Gold dimensional model.
...
All other category keys follow the general derivation shown in the integration model. Runtime validation recorded during the build confirms that the prepared product/category relationship has no unmatched keys.
...
The [business-object integration model](business_object_integration_model.webp) captures the analysis performed before the Gold star schema was built. It groups the six trusted Silver entities into three business concepts:
...
The discovery artifact is intentionally pre-modeling documentation: it explains why the three business objects were chosen before CUSTOMER and PRODUCT are represented as dimensions and SALES as the fact.
...
- Gold consumes trusted Silver keys, dates and measures; it does not repeat Silver cleansing.

The deployed model, logical relationships and final columns are documented in the [Gold star schema](../data_model/gold_star_schema.drawio), [Gold data catalog](../data_catalog/gold_data_catalog.md), and [end-to-end lineage](../data_flow/bronze_silver_gold_data_flow.drawio).
